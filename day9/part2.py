...
for line in data:
    ...
    while len(diff_set) > 1:
        sequence_increment = diff_set[-1][0]
        diff_set.remove(diff_set[-1])

        # prepend found value to the last diff_set
        diff_set[-1].insert(0, diff_set[-1][0] - sequence_increment)

    # add first element of each set to the total
    total += diff_set[0][0]
